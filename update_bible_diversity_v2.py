...
# 2. 권(Book)별 공통 교파 관점 템플릿 (인코딩 안전을 위해 키는 영문)
BOOK_TEMPLATES = {
    "moses": {
        "catholic": "율법은 구약의 성사적 기초이며, 그리스도를 향한 점진적 계시로 이해함.",
        "protestant": "율법은 죄를 깨닫게 하며, 오직 은혜로 말미암는 구원의 필요성을 역설함.",
        "orthodox": "율법은 하나님의 거룩함에 참여하는 '성화(Theosis)'의 초기 단계로 해석함.",
        "nietzsche": "율법을 강자가 약자를 다스리거나, 혹은 약자가 도덕을 통해 강자를 억압하는 수단으로 비판함."
    },
    "history": {
        "catholic": "왕권과 성전의 계승을 통해 하나님의 가시적 통치 체제를 중시함.",
        "protestant": "인간 왕들의 실패를 통해 참된 왕이신 그리스도의 필요성을 강조함.",
        "orthodox": "역사를 하나님의 섭리가 흐르는 거대한 성화의 과정으로 이해함.",
        "nietzsche": "역사적 권력 투쟁 속에서 종교적 가치가 어떻게 조작되는지 비판적으로 조명함."
    },
    "gospels": {
        "catholic": "그리스도의 인격과 성사적 현존, 교회의 가시적 일치를 중시함.",
        "protestant": "그리스도의 대속 사역과 개인의 신앙적 고백을 핵심으로 봄.",
        "orthodox": "로고스의 성육신을 통한 우주적 회복과 신성한 빛의 현현을 강조함.",
        "nietzsche": "예수를 '마지막 기독교인'으로 평가하거나, 기독교 도덕의 기원을 비판적으로 탐구함."
    }
}

def get_template(book_id):
    if book_id <= 5: return BOOK_TEMPLATES["moses"]
    if book_id <= 17: return BOOK_TEMPLATES["history"]
    if book_id >= 40: return BOOK_TEMPLATES["gospels"]
    return None
...
def process_chapter(book_name, book_id, chapter):
    ...
    if tpl:
        ...
        lines.append(f"* **가톨릭**: {tpl['catholic']}")
        lines.append(f"* **개신교**: {tpl['protestant']}")
        lines.append(f"* **동방정교회**: {tpl['orthodox']}")
        lines.append(f"* **철학적 시선 (니체 등)**: {tpl['nietzsche']}")
...
def main():
    ...
    target_books = [("요한복음", 43, 21), ("요한계시록", 66, 22), ("창세기", 1, 50), ("신명기", 5, 34), ("사무엘하", 10, 24)]
